...
export default function Suggestions({ items, remaining, onAdd }: Props) {
  ...
  return (
    <section
      ...
      className="rounded-card border border-gold/25 bg-cream-soft p-4"
    >
      <h2
      ...
      <p className="mt-1 text-[13px] leading-snug text-ink-soft">
        {single ? (
          <>
            Adding <span className="font-semibold text-ink">{items[0].name}</span> puts
            you over the line.
          </>
        ) : (
          <>
          ...
          <li key={p.id}>
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="text-xs text-ink-soft/75">{p.size}</span>
              </span>
              <span className="shrink-0 font-display text-[15px] font-extrabold text-blue">
                {peso(p.price)}
              </span>
              <span
                aria-hidden="true"
      ...
      {!single && (
        <p className="mt-2.5 text-[11.5px] text-ink-soft/70">
          Together they add {peso(addsTo)}.
        </p>
      )}
    </section>
  )
